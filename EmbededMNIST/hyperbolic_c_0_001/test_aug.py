...
slide_acc_lt = []
lung_index = {'Contain no 9':0, 'Contain 9':1}
lung_auc = {'Contain no 9':[], 'Contain 9':[]}
macro_avg_auc = []
...
for checkpoint_file in glob.glob('./*.pt'):
    ...
    test_image_data = SlideDataset.SlideDataset('../../syn/testing/bags_wo_9.txt', 
            '../../syn/testing/bags_9.txt',
            val_transform)
    test_data_loader = torch.utils.data.DataLoader(test_image_data, num_workers=7, batch_size=512)

    checkpoint = torch.load(checkpoint_file)
    minet.load_state_dict(checkpoint['minet'])
    best_epoch = checkpoint['epoch']
    best_val_auc = checkpoint['best_val_auc']
    print(f'model curvature: {minet.module.tp.c}')
...
for lung in lung_auc:
    ...
    print(f'\tFUT2: {lung}, AUC Mean: {mean}, Confidence Interval(0.95): [{lower}, {upper}]')
