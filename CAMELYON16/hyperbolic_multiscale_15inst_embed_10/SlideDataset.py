...
class SlideDataset(data.Dataset):
    ...
    def __init__(self, stage, label_file, dir_high, dir_med, dir_low, transform, fold, inst_num=10):
        '''
        read phenotypes
        '''
        self.stage = stage
    ...
    def __getitem__(self, index):
        ...
        image_lt = []
        if len(self.slide_img_high[slide_id]) < self.inst_num:
            sample_high_lt = random.choices(self.slide_img_high[slide_id], k=self.inst_num)
        else:
            sample_high_lt = random.sample(self.slide_img_high[slide_id], self.inst_num)
        
        if len(self.slide_img_med[slide_id]) < self.inst_num:
            sample_med_lt = random.choices(self.slide_img_med[slide_id], k=self.inst_num)
        else:
            sample_med_lt = random.sample(self.slide_img_med[slide_id], self.inst_num)
